...
from fuzz_cookiecutter.agent.utils.manifest_utils import (
    ...
    *,
    ...
    try:
        ...
        for index, manifest_path in enumerate(manifests):
            ...
            try:
                spec = materialize_manifest(manifest_path, workspace_dir)
                trace(
                    "batch",
                ...
                trace("batch", "manifest validation failed", case_id=case_id, error=exc)
                results.append(
                    TestResult(
                        case_id=case_id,
                        status="ERROR",
